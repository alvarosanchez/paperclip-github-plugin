...
const REMOTE_ACTION_REGISTRY_SCOPE = {
  scopeKind: 'instance' as const,
  stateKey: 'paperclip-github-plugin-remote-action-registry'
};
...
const IMPORTED_ISSUE_WAKEUP_CONCURRENCY = 1;
const MAX_REMOTE_ACTION_REGISTRY_RECORDS = 256;
const MAX_PULL_REQUEST_COMMENT_WATERMARKS = 128;
...
// Paperclip 2026.831 delivers plugin config per company (one `configChanged` per configured
// company right after startup, then on every save). The worker remembers the last delivered
// config per company so proactive paths (scheduled jobs, unscoped actions) can still resolve
// company-keyed settings when the host cannot derive a company scope for `ctx.config.get()`.
const rememberedTrustedConfigByCompanyId = new Map<string, Record<string, unknown>>();
let rememberedTrustedInstanceConfig: Record<string, unknown> | null = null;
const reportedTrustedConfigReadFailures = new Set<string>();
...
interface PendingRemoteActionWake {
  fingerprint: string;
...
interface GitHubDirectPullRequestSyncSnapshot {
  ...
  authorLogin?: string;
  commentCounts?: GitHubPullRequestCommentCountRecord;
}
...
interface GitHubTrustedPullRequestCommentSource extends GitHubPullRequestReference {
  kind: 'top_level' | 'review';
}
...
function isCompanyScopeDeniedError(error: unknown): boolean {
  const message = getErrorMessage(error).toLowerCase();
  const errorName = error instanceof Error ? error.name : '';
  return (
    errorName === 'InvocationScopeDeniedError'
    || message.includes('invocationscopedenied')
    || message.includes('company context is required')
    || message.includes('companyid is required for plugin secret resolution')
  );
}
...
const SECRET_ID_UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/iu;
...
/**
 * Plugin config and state may carry a secret id either as the Paperclip 2026.831 shared
 * `{ type: "secret_ref", secretId }` binding or as the pre-2026.831 bare id string. The worker
 * keeps the bare id internally and rebuilds the binding when it asks the host to resolve it.
 */
function normalizeSecretRef(value: unknown): string | undefined {
  if (isSecretRefBinding(value)) {
    return value.secretId.trim();
  }

  return typeof value === 'string' && value.trim() ? value.trim() : undefined;
}

function isLegacySecretRefString(value: unknown): boolean {
  return typeof value === 'string' && SECRET_ID_UUID_PATTERN.test(value.trim());
}

function toSecretRefBinding(secretId: string): EnvSecretRefBinding {
  return { type: 'secret_ref', secretId };
}

function getGitHubTokenRefConfigPath(companyId: string): string {
...
function readCompanyScopedConfigValue(config: Record<string, unknown>, key: string, companyId: string): unknown {
  const map = config[key];
  return isPlainRecord(map) ? map[companyId] : undefined;
}

/**
 * Resolves a saved secret id through the host. Paperclip 2026.831 requires the shared binding
 * object plus the company scope; `configPath` pins the resolution to the plugin config path
 * the host bound the secret at when the settings UI mirrored it.
 */
async function resolvePluginSecret(
  ...
  return ctx.secrets.resolve(toSecretRefBinding(secretId), {
    ...(companyId ? { companyId } : {}),
...
async function createUnexpectedSyncErrorResult(
  ...
  const message = isGitHubRateLimitError(error)
    ...
    : isCompanyScopeDeniedError(error)
      ? COMPANY_SCOPE_DENIED_SYNC_MESSAGE
...
function normalizePendingRemoteActionWake(value: unknown): PendingRemoteActionWake | undefined {
  if (!value || typeof value !== 'object') return undefined;
  const record = value as Record<string, unknown>;
  ...
  const mutation = record.mutation === 'import' ? 'import' : 'status_transition';
  ...
  if (!fingerprint || !assigneeAgentId || !reason) return undefined;
  return {
    fingerprint,
    assigneeAgentId,
...
function normalizeRemoteActionRegistry(value: unknown): RemoteActionRecord[] {
  if (!Array.isArray(value)) return [];
  const recordsByKey = new Map<string, RemoteActionRecord>();
  for (const entry of value) {
  ...
  if (pendingRecords.length > MAX_REMOTE_ACTION_REGISTRY_RECORDS) {
    throw new Error(
      `Remote action registry contains ${pendingRecords.length} pending wakes; maximum is ${MAX_REMOTE_ACTION_REGISTRY_RECORDS}`
    );
  }
  const retainedKeys = new Set(pendingRecords.map((record) => record.key));
  for (const record of records.sort((left, right) => left.key.localeCompare(right.key))) {
    if (retainedKeys.size >= MAX_REMOTE_ACTION_REGISTRY_RECORDS) break;
...
type EffectiveDirectPullRequestGateCondition =
  | 'conflict'
  | 'ci_failed'
  | 'review_actionable'
  | 'action_required'
...
interface EffectiveDirectPullRequestGate {
  repositoryUrl: string;
  number: number;
  headSha: string | null;
  ...
  trustedCommentKinds: Array<GitHubTrustedPullRequestCommentSource['kind']>;
  ownerCandidate?: string;
  pullRequest: GitHubPullRequestStatusSnapshot;
}

function getEffectiveDirectPullRequestGatePriority(condition: EffectiveDirectPullRequestGateCondition): number {
  switch (condition) {
    case 'conflict': return 0;
    case 'ci_failed': return 1;
    case 'review_actionable': return 2;
    case 'action_required': return 3;
    case 'trusted_comment': return 4;
...
function buildTrustedCommentKindsByPullRequest(
  trustedCommentSources: GitHubTrustedPullRequestCommentSource[]
): Map<string, Array<GitHubTrustedPullRequestCommentSource['kind']>> {
  const kindsByPullRequest = new Map<string, Set<GitHubTrustedPullRequestCommentSource['kind']>>();
  for (const source of trustedCommentSources) {
    const key = buildGitHubPullRequestReferenceKey(source);
    const kinds = kindsByPullRequest.get(key) ?? new Set<GitHubTrustedPullRequestCommentSource['kind']>();
    kinds.add(source.kind);
    kindsByPullRequest.set(key, kinds);
  }
  return new Map([...kindsByPullRequest].map(([key, kinds]) => [key, [...kinds].sort()]));
}
...
function selectEffectiveDirectPullRequestGate(
  ...
  return selectHighestPriorityEffectivePullRequestGate(pullRequests
    .filter((entry): entry is GitHubDirectPullRequestSyncSnapshot & { pullRequest: GitHubPullRequestStatusSnapshot } =>
      entry.lifecycleState === 'open' && Boolean(entry.pullRequest)
    )
    .map((entry): EffectiveDirectPullRequestGate => {
      const repositoryUrl = getNormalizedMappingRepositoryUrl({ repositoryUrl: entry.repository.url }).toLowerCase();
      const number = entry.pullRequestLink.data.githubPullRequestNumber;
      ...
      return {
        repositoryUrl,
        number,
        headSha: entry.pullRequest.headSha ?? null,
        condition,
        remoteCondition,
        trustedCommentKinds,
...
function selectEffectiveIssueLinkedPullRequestGate(
  ...
  const ownerByPullRequest = new Map(pullRequestLinks.map((link) => [
    buildGitHubPullRequestReferenceKey({
      ...
      number: link.data.githubPullRequestNumber
    }),
    link.data.followThroughAssigneeAgentId
  ]));

  return selectHighestPriorityEffectivePullRequestGate(pullRequests.map((pullRequest) => {
    const repositoryUrl = getNormalizedMappingRepositoryUrl({ repositoryUrl: pullRequest.repositoryUrl }).toLowerCase();
    const number = pullRequest.number;
    const key = buildGitHubPullRequestReferenceKey({ repositoryUrl, number });
    const trustedCommentKinds = trustedCommentKindsByPullRequest.get(key) ?? [];
    const { condition, remoteCondition } = getEffectiveDirectPullRequestGateCondition(
      pullRequest,
      hasTrustedIssueComment || trustedCommentKinds.length > 0
    );
    return {
      repositoryUrl,
      number,
...
function selectEffectiveIssueLinkedPullRequestAction(
  ...
  return selectEffectiveIssueLinkedPullRequestGate(
    pullRequests,
...
function isActionableDirectPullRequestGate(
  gate: EffectiveDirectPullRequestGate | undefined
): boolean {
  return Boolean(gate && gate.condition !== 'ready' && gate.condition !== 'waiting');
}
...
function resolvePaperclipIssueStatus(params: {
  ...
  if (hasTrustedNewComment && snapshot.linkedPullRequests.length === 0) {
  ...
  if (effectiveGate) {
    ...
    if (isActionableDirectPullRequestGate(effectiveGate)) {
      if (shouldPreserveBlockedExternalPullRequestWait({
        currentStatus,
        linkedPullRequests: [effectiveGate.pullRequest]
      })) {
        return 'blocked';
      }
      return hasExecutorHandoffTarget ? 'in_progress' : 'todo';
  ...
  if (effectiveGate) {
    if (effectiveGate.condition === 'trusted_comment') {
      return currentStatus === 'backlog'
        ? 'backlog'
        : hasExecutorHandoffTarget ? 'in_progress' : 'todo';
    }
    if (isActionableDirectPullRequestGate(effectiveGate)) {
      if (shouldPreserveBlockedExternalPullRequestWait({
        currentStatus,
...
function tryBuildGitHubPullRequestStatusSnapshotFromBatchNode(node: {
  ...
  return {
    number: node.number,
    repositoryUrl: repository.url,
    ...(typeof node.headRefOid === 'string' && node.headRefOid.trim()
      ? { headSha: node.headRefOid.trim().toLowerCase() }
      : {}),
...
async function getGitHubPullRequestCiSnapshot(
  ...
  do {
    ...
    const responseHeadSha = response.repository?.pullRequest?.headRefOid;
    if (typeof responseHeadSha === 'string' && responseHeadSha.trim()) {
      headSha = responseHeadSha.trim().toLowerCase();
    }
...
async function getGitHubPullRequestStatusSnapshot(
  ...
  const loadSnapshotPromise = (async () => {
    ...
    return cacheGitHubPullRequestStatusSnapshot(repository, {
      number: pullRequestNumber,
      repositoryUrl: repository.url,
      ...('headSha' in ciSnapshot && ciSnapshot.headSha ? { headSha: ciSnapshot.headSha } : {}),
      hasUnresolvedReviewThreads: reviewThreadSummary.unresolvedReviewThreads > 0,
...
async function listTrustedNewLinkedPullRequestCommentSources(params: {
  ...
  for (const currentCommentCount of params.currentCommentCounts) {
    ...
    const repositoryUrl = getNormalizedMappingRepositoryUrl({ repositoryUrl: currentCommentCount.repositoryUrl });
    ...
    if (currentCommentCount.topLevelCommentCount > previousCommentCount.topLevelCommentCount) {
      ...
      if (await hasTrustedGitHubCommentRecords({
        ...
        originalPosterLogin: params.originalPosterLogin,
        maintainerCache: params.maintainerCache
      })) {
        sources.push({ repositoryUrl, number: currentCommentCount.number, kind: 'top_level' });
      }
    }
    ...
    if (currentCommentCount.reviewCommentCount > previousCommentCount.reviewCommentCount) {
      ...
      if (await hasTrustedGitHubCommentRecords({
        ...
        sources.push({ repositoryUrl, number: currentCommentCount.number, kind: 'review' });
      }
    }
  }
...
function normalizeGitHubPullRequestCommentCountRecords(value: unknown): GitHubPullRequestCommentCountRecord[] {
  ...
  }).slice(0, MAX_PULL_REQUEST_COMMENT_WATERMARKS);
}
...
function removeGitHubIssueLinkMetadataFromDescription(description: string | null | undefined): string | undefined {
  if (typeof description !== 'string') {
    return undefined;
  }
  ...
  next = next
    .replace(/^\*\s+GitHub issue:\s+\[[^\]]+\]\([^)]+\)\s*$/gm, '')
    .replace(/^Imported from https:\/\/github\.com\/[^\s)]+\/[^\s)]+\/issues\/\d+\.?\s*$/gim, '')
    .replace(/[ \t]+\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();

  return next === description ? undefined : next;
}
...
async function tombstoneGitHubIssueLinkRecord(
  ctx: PluginSetupContext,
  record: GitHubIssueLinkRecord,
  unlinkedAt: string
): Promise<void> {
...
async function tombstoneGitHubPullRequestLinkRecord(
  ctx: PluginSetupContext,
  record: GitHubPullRequestLinkRecord,
  unlinkedAt: string
): Promise<void> {
...
async function unlinkPaperclipIssueFromGitHub(
  ctx: PluginSetupContext,
  params: {
    companyId: string;
    issueId: string;
  ...
  const importRegistry = normalizeImportRegistry(await ctx.state.get(IMPORT_REGISTRY_SCOPE));
  const removedImportRegistryEntries = importRegistry.filter((entry) =>
    entry.paperclipIssueId === issueId && (!entry.companyId || entry.companyId === companyId)
  );
  const nextImportRegistry = importRegistry.filter((entry) =>
    !(entry.paperclipIssueId === issueId && (!entry.companyId || entry.companyId === companyId))
  );
  ...
  for (const record of issueLinkRecords) {
    ...
    invalidateGitHubLinkCachesForTarget({
      ...
      projectId: record.data.paperclipProjectId ?? issue.projectId ?? undefined,
      repositoryUrl: record.data.repositoryUrl
    });
  }
  ...
  for (const entry of removedImportRegistryEntries) {
    invalidateGitHubLinkCachesForTarget({
      ...
      projectId: entry.paperclipProjectId ?? issue.projectId ?? undefined,
  ...
  return {
    paperclipIssueId: issueId,
    unlinked: issueLinkRecords.length > 0
      || pullRequestLinkRecords.length > 0
      || removedImportRegistryEntries.length > 0
      || shouldClearGitHubOrigin
    ...
    removedDescriptionMetadata: nextDescription !== undefined
  };
}
...
async function wakePaperclipIssueAssignee(
    ...
    try {
      const result = await ctx.issues.requestWakeup(params.paperclipIssueId, params.companyId, {
        ...
        ...(params.actionFingerprint
          ? { idempotencyKey: ['github-sync', params.mutation, params.paperclipIssueId, params.actionFingerprint].join(':') }
      ...
      if (!params.paperclipApiBaseUrl) {
        throw new Error(
          `GitHub sync could not wake the assignee for Paperclip issue ${params.paperclipIssueId} through the SDK: ${getErrorMessage(error)}`,
          { cause: error }
        );
      }
  ...
  if (!params.paperclipApiBaseUrl) {
    throw new Error(`GitHub sync has no available wakeup transport for Paperclip issue ${params.paperclipIssueId}.`);
  }
...
function getEffectivePullRequestActionCondition(
  pullRequest: GitHubPullRequestStatusSnapshot
): 'conflict' | 'ci_failed' | 'review_actionable' | 'ready' | 'waiting' {
  if (pullRequest.mergeability === 'conflicting' || pullRequest.mergeStateStatus === 'dirty') return 'conflict';
  if (pullRequest.ciState === 'red') return 'ci_failed';
  if (pullRequest.hasUnresolvedReviewThreads) return 'review_actionable';
  if (pullRequest.ciState === 'green' && pullRequest.mergeability === 'mergeable') return 'ready';
  return 'waiting';
}

function buildRemoteActionFingerprint(
  snapshot: GitHubIssueStatusSnapshot,
  effectiveGate = snapshot.state === 'closed'
    ? undefined
    : selectEffectiveIssueLinkedPullRequestGate(snapshot.linkedPullRequests, [], false)
): string {
  const gate = snapshot.state === 'closed' || !effectiveGate
    ? null
    : {
  ...
  return createHash('sha256').update(JSON.stringify({
    ...
    issue: {
      ...
      stateReason: snapshot.stateReason ?? null
    },
    gate,
    reasonCode
  })).digest('hex');
...
function buildDirectPullRequestActionFingerprint(
  snapshots: GitHubDirectPullRequestSyncSnapshot[],
  effectiveGate: EffectiveDirectPullRequestGate | undefined
): string {
  const terminalOutcome = effectiveGate
    ? null
    : snapshots.some((entry) => entry.lifecycleState === 'merged') ? 'merged' : 'closed';
...
function getGitHubIssueRepositoryReference(issue: GitHubIssueRecord): ParsedGitHubIssueReference | undefined {
  return parseGitHubIssueHtmlUrl(issue.htmlUrl);
}

function findTransferredIssueTargetMapping(
  mappings: RepositoryMapping[],
  params: {
    companyId?: string;
    repositoryUrl: string;
...
async function patchPaperclipIssueForGitHubIssueTransfer(
  ctx: PluginSetupContext,
  params: {
    companyId: string;
    issueId: string;
  ...
  if (!issueUpdated) {
    await ctx.issues.update(
      params.issueId,
      patch as unknown as PaperclipIssueUpdatePatchWithLabels,
      params.companyId
    );
  }
...
async function moveImportedIssueToTransferredGitHubMapping(
  ctx: PluginSetupContext,
  params: {
    sourceMapping: RepositoryMapping;
    targetMapping: RepositoryMapping;
    importedIssue: ImportedIssueRecord;
    githubIssue: GitHubIssueRecord;
    nextRegistry: ImportedIssueRecord[];
    paperclipApiBaseUrl?: string;
  }
  ...
  const projectId = normalizeOptionalString(params.targetMapping.paperclipProjectId);
  if (!companyId || !projectId) {
    return {
  ...
  for (const link of existingLinks) {
    if (
      (!link.data.companyId || link.data.companyId === companyId)
      && link.data.githubIssueUrl !== nextGitHubIssueUrl
    ) {
  ...
  invalidateGitHubLinkCachesForTarget({
    companyId,
    projectId: params.sourceMapping.paperclipProjectId ?? paperclipIssue.projectId ?? undefined,
...
function buildUnmappedTransferredIssueCancellationComment(params: {
  previousStatus: PaperclipIssueStatus;
  nextStatus: PaperclipIssueStatus;
  transferredRepository: ParsedRepositoryReference;
}): string {
...
async function cancelUnmappedTransferredGitHubIssue(
  ctx: PluginSetupContext,
  params: {
    mapping: RepositoryMapping;
    importedIssue: ImportedIssueRecord;
...
async function fetchTransferredGitHubIssueForImportedRecord(
  octokit: Octokit,
  repository: ParsedRepositoryReference,
  importedIssue: ImportedIssueRecord
): Promise<{
  ...
  if (importedIssue.githubIssueNumber === undefined) {
    return null;
  }
  ...
  const transferredRepository = transferredIssueReference
    ? parseRepositoryReference(transferredIssueReference.repositoryUrl)
    : undefined;
...
async function reconcileTransferredImportedIssues(
  ctx: PluginSetupContext,
  params: {
    octokit: Octokit;
    sourceMapping: RepositoryMapping;
    sourceRepository: ParsedRepositoryReference;
    allMappings: RepositoryMapping[];
  ...
  for (const importedIssue of params.importedIssues) {
    ...
    try {
      ...
      for (let index = params.nextRegistry.length - 1; index >= 0; index -= 1) {
        ...
        if (
          entry?.paperclipIssueId === importedIssue.paperclipIssueId &&
          (!entry.companyId || entry.companyId === params.sourceMapping.companyId)
        ) {
...
async function synchronizePaperclipIssueStatuses(
  ...
  syncFailureContext: SyncFailureContext,
  failures: SyncProcessingFailure[],
  persistImportRegistry: () => Promise<void>,
  ...
  for (const importedIssue of importedIssues) {
      ...
      const issueLinkedPullRequestLinks = snapshot.linkedPullRequests.length === 0
        ? []
        : await listGitHubPullRequestLinkRecords(ctx, {
            paperclipIssueId: importedIssue.paperclipIssueId
          });
      const effectiveGate = selectEffectiveIssueLinkedPullRequestGate(
        snapshot.linkedPullRequests,
      ...
      if (paperclipIssue.status === nextStatus) {
        ...
        if (pendingWake) {
          ...
          await persistObservedRemoteState();
        }
        continue;
      }
  ...
  await mapWithConcurrency(
    ...
    async (queuedWakeup) => {
      const admitted = await wakePaperclipIssueAssignee(ctx, {
        ...
        actionFingerprint: queuedWakeup.actionFingerprint,
...
async function synchronizePaperclipPullRequestIssueStatuses(
  ...
  syncFailureContext: SyncFailureContext,
  failures: SyncProcessingFailure[],
  remoteActionRegistry: RemoteActionRecord[],
  persistRemoteActionRegistry: () => Promise<void>,
  assertNotCancelled?: () => Promise<void>,
  onProgress?: (progress: {
    pullRequestLink: GitHubPullRequestLinkRecord;
    completedIssueCount: number;
  ...
  for (const [paperclipIssueId, issuePullRequestLinks] of pullRequestLinkGroups.entries()) {
    ...
    for (const pullRequestLink of issuePullRequestLinks) {
      ...
      try {
        ...
        const directPullRequestMetadata = {
          ...(normalizeGitHubUsername(pullRequestResponse.data.user?.login)
            ? { authorLogin: normalizeGitHubUsername(pullRequestResponse.data.user?.login) }
            : {}),
          commentCounts: {
            number: pullRequestLink.data.githubPullRequestNumber,
            repositoryUrl: pullRequestRepository.url,
            topLevelCommentCount:
              typeof pullRequestResponse.data.comments === 'number' && pullRequestResponse.data.comments >= 0
                ? Math.floor(pullRequestResponse.data.comments)
                : 0,
            reviewCommentCount:
              typeof pullRequestResponse.data.review_comments === 'number' && pullRequestResponse.data.review_comments >= 0
                ? Math.floor(pullRequestResponse.data.review_comments)
                : 0
          }
        };
    ...
    try {
      ...
      const remoteActionKey = `direct-pr:${mapping.companyId}:${paperclipIssueId}`;
      let remoteAction = remoteActionRegistry.find((entry) => entry.key === remoteActionKey);
      if (!remoteAction) {
      ...
      const currentCommentCounts = pullRequestSnapshots
        ...
        .filter((counts): counts is GitHubPullRequestCommentCountRecord => Boolean(counts));
      const trustedCommentSources: GitHubTrustedPullRequestCommentSource[] = [];
      if (paperclipIssue.status !== 'backlog' && remoteAction.linkedPullRequestCommentCounts) {
        const maintainerCache = new Map<string, boolean>();
        for (const snapshot of pullRequestSnapshots) {
          if (!snapshot.commentCounts) continue;
          trustedCommentSources.push(...await listTrustedNewLinkedPullRequestCommentSources({
            octokit,
            originalPosterLogin: snapshot.authorLogin,
            previousCommentCounts: remoteAction.linkedPullRequestCommentCounts,
            currentCommentCounts: [snapshot.commentCounts],
            maintainerCache
          }));
        }
      }
...
function rememberTrustedConfig(companyId: string | null | undefined, value: unknown): void {
  const record = isPlainRecord(value) ? { ...value } : {};
  const normalizedCompanyId = normalizeCompanyId(companyId);
  if (normalizedCompanyId) {
    rememberedTrustedConfigByCompanyId.set(normalizedCompanyId, record);
  } else {
    rememberedTrustedInstanceConfig = record;
  }
}

function getRememberedConfiguredCompanyIds(): string[] {
  return [...rememberedTrustedConfigByCompanyId.keys()].sort((left, right) => left.localeCompare(right));
}

/**
 * Merges every delivered company config into one instance-wide view. Company-keyed maps such as
 * `githubTokenRefs` combine across companies; scalar keys keep the first delivered value.
 */
function mergeRememberedTrustedConfig(): Record<string, unknown> {
  const merged: Record<string, unknown> = { ...(rememberedTrustedInstanceConfig ?? {}) };
  for (const companyId of getRememberedConfiguredCompanyIds()) {
    const config = rememberedTrustedConfigByCompanyId.get(companyId) ?? {};
...
async function readTrustedConfig(
  ...
  try {
    const value = await ctx.config.get(normalizedCompanyId);
    const record = isPlainRecord(value) ? value : {};
    if (normalizedCompanyId) {
      rememberedTrustedConfigByCompanyId.set(normalizedCompanyId, { ...record });
    }
    ...
    if (!reportedTrustedConfigReadFailures.has(warningKey)) {
      ...
      ctx.logger.warn(
        'GitHub Sync could not read company-scoped plugin config from the Paperclip host; using the last config the host delivered for this scope.',
        {
          companyId: normalizedCompanyId ?? null,
          usingDeliveredConfig: Boolean(remembered && Object.keys(remembered).length > 0),
          error: getErrorMessage(error)
        }
      );
    }
...
function getConfiguredGithubTokenSource(
  ...
  const companyScopedSecretRef = normalizedCompanyId
...
async function resolveGithubToken(
  ...
  const config = options.config ?? await getResolvedConfig(ctx, options.companyId);
  ...
  if (configuredTokenSource.secretRef) {
    try {
      const token = (await resolvePluginSecret(ctx, configuredTokenSource.secretRef, {
        companyId: options.companyId,
        ...(configuredTokenSource.configPath ? { configPath: configuredTokenSource.configPath } : {})
      })).trim();
      if (token) {
        return token;
      }
...
async function buildProjectPullRequestsPageData(
  ...
  const config = await getResolvedConfig(ctx, scope.companyId);
...
async function performSync(
  ...
  const config = await getResolvedConfig(ctx, targetCompanyId);
  const settings = materializeScopedSettings(baseSettings, config, targetCompanyId);
  const importRegistry = normalizeImportRegistry(await ctx.state.get(IMPORT_REGISTRY_SCOPE));
  const remoteActionRegistry = normalizeRemoteActionRegistry(await ctx.state.get(REMOTE_ACTION_REGISTRY_SCOPE));
  ...
  // Synchronization decisions must observe current remote action signals. The
  // longer-lived UI insight caches are useful within a run, but reusing them
  // across runs can hide a new head, check result, or review action.
  activeGitHubPullRequestStatusSnapshotCache.clear();
  activeGitHubPullRequestStatusSnapshotPromiseCache.clear();
  activeGitHubPullRequestReviewThreadSummaryCache.clear();
  activeGitHubPullRequestReviewThreadSummaryPromiseCache.clear();
  ...
  try {
    ...
    for (const plan of repositoryPlans) {
      ...
      try {
        ...
        const transferReconciliationResult = await reconcileTransferredImportedIssues(ctx, {
          ...
          importedIssues: [...importRegistryByIssueId.values()].filter((importedIssue) =>
        ...
        const synchronizationResult = await synchronizePaperclipIssueStatuses(
          ...
          failureContext,
          recoverableFailures,
          async () => ctx.state.set(IMPORT_REGISTRY_SCOPE, nextRegistry),
        ...
        const pullRequestSynchronizationResult = await synchronizePaperclipPullRequestIssueStatuses(
          ...
          failureContext,
          recoverableFailures,
          remoteActionRegistry,
          async () => {
            const compacted = normalizeRemoteActionRegistry(remoteActionRegistry);
            remoteActionRegistry.splice(0, remoteActionRegistry.length, ...compacted);
            await ctx.state.set(REMOTE_ACTION_REGISTRY_SCOPE, remoteActionRegistry);
          },
          throwIfSyncCancelled,
          async (progress) => {
    ...
    for (const [externalIssueIndex, issueLink] of externalSyncWork.issueLinks.entries()) {
      ...
      try {
        ...
        const existingImportedIssue = nextRegistry.find((entry) =>
          entry.paperclipIssueId === issueLink.paperclipIssueId
          && entry.companyId === companyId
          && entry.repositoryUrl === repository.url
        );
        const importedIssue = existingImportedIssue ?? upsertImportedIssueRecord(nextRegistry, {
        ...
        if (!existingImportedIssue) {
          // Entity-only links previously used a temporary record while persisting
          // an unrelated registry array. Establish the durable baseline before
          // any status action so failures and worker restarts cannot consume a
          // trusted comment or lose a pending wake.
...
const plugin = definePlugin({
  // GitHub Sync serves every company on the instance from one worker; Paperclip 2026.831 delivers
  // one company-scoped config per configured company and rejects that fan-out unless the plugin
  // declares it keys config by company.
  multiCompanyConfig: true,
  async onConfigChanged(newConfig, context) {
    const companyId = normalizeCompanyId(context?.companyId) ?? null;
  ...
  async setup(ctx) {
    ...
    ctx.data.register('settings.registration', async (input) => {
      ...
      const [config, rawTrustedConfig] = await Promise.all([
        getResolvedConfig(ctx, requestedCompanyId),
        readTrustedConfig(ctx, requestedCompanyId)
      ]);
      const trustedConfig = normalizeConfig(rawTrustedConfig);
      ...
      // Paperclip 2026.831 only binds (and resolves) secret refs stored as `{ type: "secret_ref" }`
      // objects; a bare UUID left behind by an older release must be mirrored again by the UI.
      const configuredGitHubTokenRefIsLegacy = Boolean(
        requestedCompanyId
        && isLegacySecretRefString(readCompanyScopedConfigValue(rawTrustedConfig, 'githubTokenRefs', requestedCompanyId))
      );
      const configuredBoardTokenRefIsLegacy = Boolean(
        requestedCompanyId
        && isLegacySecretRefString(
          readCompanyScopedConfigValue(rawTrustedConfig, 'paperclipBoardApiTokenRefs', requestedCompanyId)
        )
      );
      ...
      return {
        ...
        githubTokenNeedsConfigSync: Boolean(
          savedGitHubTokenRef
          && (configuredGitHubTokenRef !== savedGitHubTokenRef || configuredGitHubTokenRefIsLegacy)
        ),
        ...(savedBoardTokenRef ? { paperclipBoardAccessConfigSyncRef: savedBoardTokenRef } : {}),
        paperclipBoardAccessNeedsConfigSync: Boolean(
          savedBoardTokenRef
          && (configuredBoardTokenRef !== savedBoardTokenRef || configuredBoardTokenRefIsLegacy)
        )
      };
    });
