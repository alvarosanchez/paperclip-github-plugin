...
- The plugin MUST expose agent tools for the GitHub issue and pull request workflow around synced work, including repository-item search, issue reads and updates, `assign_to_current_user` for assigning issues to the GitHub user authenticated by the saved token, issue comment reads and writes, pull request creation and updates, pull request file and CI inspection, review-thread reads and replies, review-thread resolution changes, `request_pull_request_reviewers` reviewer requests, organization-level GitHub Project search/listing, and associating pull requests with organization-level GitHub Projects.
...
- When a linked GitHub issue is transferred to another repository, sync MUST detect the canonical issue URL returned by GitHub. If the destination repository is mapped to a Paperclip project in the same company, sync MUST move the existing Paperclip issue to that project, update the active GitHub link metadata and import-registry row, and avoid creating a duplicate Paperclip issue. If the destination repository is not mapped, sync MUST unlink the Paperclip issue from GitHub and transition it to `cancelled` with an explanatory Paperclip comment.
...
- Repeated sync runs MUST also monitor open pull requests that were linked to Paperclip issues through the project Pull Requests page, the `create_pull_request` agent tool, the agent-authenticated metric API route, or the manual issue-page fallback when those pull requests do not close a synced GitHub issue, and MUST reconcile those Paperclip issue statuses from the pull request CI, merge, review decision, and review-thread state.
...
- If a real Paperclip host creates a newly imported maintainer-authored issue in `backlog` before first-sync reconciliation promotes it to `todo`, that promotion MUST preserve the imported default assignee instead of applying the configured executor handoff assignee.
...
- An open GitHub issue with a linked PR that still has unfinished CI jobs MUST map to Paperclip `in_progress`, except when the Paperclip issue is already `blocked` and the linked PR has no unresolved review feedback, merge conflict, out-of-date branch, or draft state; in that case sync MUST preserve `blocked` instead of waking an executor.
...
- An open GitHub issue with a linked PR that has green CI, a review-ready GitHub merge state such as `CLEAN` or `HAS_HOOKS`, and all review threads resolved MUST map to Paperclip `in_review`; aggregate GitHub `CHANGES_REQUESTED` alone MUST NOT move that wait back to active execution without an actionable unresolved thread, failing or pending CI, action-required merge state, draft state, or trusted new comment.
- An imported Paperclip issue that is currently `blocked` and has any non-terminal `blockedBy` issue MUST remain `blocked` during GitHub Sync even when the linked GitHub issue or PR otherwise looks review-ready.
- An open GitHub issue with a linked PR in GitHub merge state `UNKNOWN` MUST NOT be treated as review-ready on first import or while the issue is already in active execution. When an imported Paperclip issue is already `in_review`, or is being reopened from `done` with a linked PR, transient `UNKNOWN` mergeability MUST preserve or restore `in_review` if CI is green, all review threads are resolved, and no trusted new comment, draft state, explicit action-required merge state, or unresolved first-class blocker applies.
...
- When sync moves an imported Paperclip issue into `done` or `cancelled`, or sees an already-terminal imported issue whose GitHub source is still terminal, it MUST clear any pending Paperclip review or approval execution policy/state as part of that same reconciliation so the host does not reject the terminal status update or keep waking stale review participants.
...
- The issue detail contribution SHOULD hide itself only when it cannot resolve the current Paperclip issue context. When the current Paperclip issue has no linked GitHub issue or pull request, it SHOULD render a compact manual-link fallback instead of an inert empty state.
- The issue detail contribution SHOULD allow operators to manually link an unlinked Paperclip issue to either a GitHub issue or a GitHub pull request by entering either a number or a full GitHub URL. Number-only entries SHOULD resolve against the issue's mapped Paperclip project repository; full URLs SHOULD be allowed when the referenced repository is mapped to that project.
- A manual GitHub issue link MUST write the same issue-link entity and import-registry entry used by normal GitHub issue imports. A manual GitHub pull request link MUST write the same pull-request link entity used by project Pull Requests page create/link flows.
- The issue detail contribution SHOULD allow operators to unlink a linked Paperclip issue from GitHub. Unlinking MUST remove the link from active worker resolution, remove any matching import-registry entry, clear GitHub Sync origin targets and hidden import markers from the Paperclip issue where possible, and leave both the Paperclip issue and GitHub item intact.
