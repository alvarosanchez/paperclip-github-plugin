...
- GitHub Sync will not repeatedly re-open a deliberately blocked issue. When it moves an issue out of `blocked` it records a hash of the linked pull requests' externally observable state (head SHA, CI state, mergeability, merge state status, review decision, unresolved review threads); while that state is unchanged, a later pass leaves `blocked` alone. This matters when a required check is red for something nobody in the company can clear — the contributor CLA gate is the common case, an upstream baseline failure or an unreleased dependency behave the same. Previously each sync pass moved the issue back to active work, the assignee woke, re-established that it was still externally blocked, and the loop repeated on the next pass. Any real change — a push, a CI result, a review, a resolved conflict — changes the hash and normal routing resumes immediately.
