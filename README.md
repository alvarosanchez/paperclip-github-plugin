...
- `done` is terminal for the exact acknowledged open-PR snapshot, not shorthand for merged. GitHub Sync persists the review-ready repository, PR number, and head SHA before a completion can be acknowledged; a later green snapshot preserves `done` only when all three match. A changed or missing head comparison, failed CI, blocking review state, conflict/action-required state, or trusted follow-up reopens accountable active work through the durable follow-through owner or normal execution routing. Repeated matching terminal snapshots do not mutate status, assignment, execution state, comments, or wake records.
